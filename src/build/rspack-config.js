...
export async function createRspackConfig({
  ...
  // `mode` is passed in by the caller (`aplos build --mode` is the source of
  // truth; the root rspack.config.js adapter reads it from APLOS_MODE for CLI
  // use). Historically reading NODE_ENV first made `--mode=production` silently
  // emit a development bundle (no contenthash, so deploys serve stale files from
  // HTTP caches) whenever the environment set NODE_ENV to anything else, which
  // CI runners routinely do. Keeping mode an explicit parameter avoids that.
  ...
  const defaultTemplate = path.resolve(frameworkRoot, "./src/client/public/index.html");
  ...
  // Guard against loading the framework's own config by accident: only load a
  // config that lives in the project directory and is not this framework file.
  let userConfig = {};
  const userConfigPath = path.resolve(projectDirectory, "rspack.config.js");
  if (fs.existsSync(userConfigPath) && userConfigPath !== path.resolve(frameworkRoot, "rspack.config.js")) {
    try {
  ...
  const frameworkConfig = {
    ...
    resolve: {
      alias: {
        ...
        "aplos/internal/default-middleware": path.resolve(frameworkRoot, "src/runtime/default-middleware.js"),
  ...
  // project. They are applied AFTER the merge so a project's rspack.config.js
  // cannot override them: otherwise a user config could turn a production build
  // back into a development one, or replace the framework entry point.
